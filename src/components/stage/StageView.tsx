...
function parseAgentMessage(text: string): ParsedBlock[] {
  ...
  while ((match = pattern.exec(text)) !== null) {
    ...
    switch (blockType) {
      case 'project':
        blocks.push({ type: 'content', value: blockData, contentType: 'project', props: { projectId: blockData } });
        break;
      case 'infra':
        blocks.push({ type: 'content', value: 'infra', contentType: 'infra' });
        break;
      case 'stat': {
        const [value, label, context] = blockData.split('|');
        blocks.push({ type: 'content', value: 'stat', contentType: 'stat', props: { value, label, context } });
        break;
      }
      case 'philosophy':
        blocks.push({ type: 'content', value: blockData, contentType: 'philosophy', props: { quote: blockData } });
        break;
      case 'cta':
